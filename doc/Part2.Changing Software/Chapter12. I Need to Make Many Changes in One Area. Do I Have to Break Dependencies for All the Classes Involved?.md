...
## 조임 지점
#### 조임 지점 : 영향 스케치에서 영향이 집중되는 곳  
조임 지점을 찾게 되면 변경 작업은 매우 쉬워진다.  
조임 지점은 변경 지점에 의해 결정된다.  

### 조임 지점 찾기
#### 조임 지점이란 영향 스케치 내에서 영향이 집중되는 곳이며, 
#### 소수 메소드에 대응하는 테스트 루틴으로 다수 메소드의 변경을 검출할 수 있는 곳이다.

* 조임 지점 찾기가 어렵다면 ???  
  > 영향을 미치는 것이 매우 많을 경우  
  > 영향 스케치는 매우 복잡해진다.  
  * 한 번에 너무 많은 변경을 시도하지 않는 지 확인한다.  
    > 가급적 개별 변경 지점 근처에서 테스트 루틴을 작성하려 노력한다.  
  * 영향 스케치 내에서 공통적인 사용 방법을 찾는다.  
    > == 메시지를 전달하는 인터페이스 단위로 생각  
    
  // 더 복잡한 경우에 고려해야 할 내용도 있었지만(p256 마지막 ~), 다음 단원과 중복되 내용    
  
### 조임 지점을 이용한 설계 판단  
조임 지점의 위치는 코드 개선에 대한 힌트를 준다.  

#### 조임 지점은 자연적인 캡슐화의 경계다.(캡슐화의 정의와 연관)  
> BillingStatement.makeStatement() 를 호출 하기 위해  
> Invoice 와 Item 객체를 알아야 할 필요는 없다.  

#### 조임 지점을 통해 클래스 간에 책임 어떻게 분배해야 더 나은 캡슐화가 될지 파악할 수 있다.  
// 조임 지점과 DDD의 AGGREGATE 패턴 속 root 와 연관지어 생각해 볼 수 있다.  

### 조임 지점의 함정
#### 단위 테스트가 점점 소규모 통합 테스트로 커지는 상황
> 테스트 루틴 수행 비용 증가 (실행 시간)  

* 새로운 코드를 위한 단위 테스트  
> 가급적 독립적으로 클래스를 테스트한다.  
  * 테스트 루틴이 너무 큰 것 같다면 다시 조사하여 좀 더 작은 단위로 분리한다.  
    #### 단위 테스트의 목적은 하나의 객체가 어떻게 동작하는지 확인하는 것  
    #### 캡슐화한 객체를 Fake Object로 대체하는 이유를 생각  
    
* 기존 코드를 위한 테스트 루틴  
#### 애플리케이션의 일부를 추출해서 그 부분에 대한 테스트 루틴을 작성하는 것이 이득일 때도 있다.
#### 위 과정을 통해 단위 테스트 대상을 추출하는 작업이 쉬워진다.

### 최종 목적지 : 조임 지점의 테스트 루틴을 삭제하고 각각의 클래스에 대한 단위 테스트를 적용해 개발 진행
